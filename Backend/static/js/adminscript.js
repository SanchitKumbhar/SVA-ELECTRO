...
async function fetchJWTToken() {
    try {
        const res = await fetch('/get-jwt', {
            method: 'GET',
            credentials: 'include', // needed if token is in an HttpOnly cookie
        });

        if (!res.ok) throw new Error('JWT fetch failed');

        const data = await res.json();
        return data.access; // assuming response = { access: "JWT_TOKEN" }
    } catch (err) {
    ...
    try {
        ...
        data.forEach(app => {
            ...
            row.innerHTML = `
                ...
                <td>${app.qty}</td>
                <td>${app.fromappointmentdate}</td>
                <td>${app.toappointmentdate}</td>
                <td>${app.location}</td>
                <td>${app.purpose}</td>
                <td>${app.message}</td>
                <td><button onclick="check('${app.id}')">Check</button></td>
            `;
...
// Trigger when navigating to appointments tab
...
// Show slot modal with dates
function showSlotModal(dates) {
  const modal = document.getElementById('slot-modal');
  const closeBtn = document.getElementById('close-slot-modal');
  const toggleBtn = document.getElementById('toggle-dates-btn');
...
// Update your check function to call showSlotModal
async function check(params) {
  ...
  try {
    const res = await fetch(`/api/appointment/${params}/checkslot/`, {
      headers: {
        'Authorization': `Bearer ${jwt}`,
        'Content-Type': 'application/json'
      },
    ...
    if (!res.ok) return new Error("fetch failed");

    const data = await res.json();
    // Show modal with dates
    showSlotModal(data.message || []);
  }
  catch(err){
    console.log(err);
  }
